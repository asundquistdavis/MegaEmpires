...
const useAppState = () => {
    ...
    useEffect(()=> {
        if (appState.game) {
            ...
            if (appState.game.status==='playing') {
                setAppState(state=>({...state, ...PlayGameState.new(setAppState, appState.game, appState.user)}))
            }
        }
    }, [appState.game]);
